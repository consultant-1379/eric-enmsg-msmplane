...
pipeline {
    ...
    stages {
        ...
        stage('Checkout Cloud-Native SG Git Repository') {
            steps {
                git branch: 'master',
                        url: 'ssh://gerrit-gamma.gic.ericsson.se:29418/OSS/com.ericsson.oss.containerisation/eric-enmsg-msmplane'
            }
        }
        stage('Helm Dep Up ') {
            steps {
        ...
        stage('Merge values files') {
            steps{
                 script {
                     appconfig_values = sh (script: "ls ${WORKSPACE}/chart/eric-enmsg-msmplane/appconfig/ | grep values.yaml", returnStatus: true)
                     ...
                     sh '''
                         if git status | grep 'values.yaml' > /dev/null; then
                            git add chart/eric-enmsg-msmplane/values.yaml
